"""Analyse how often agents visit barriers introduced in previous worlds.

This script reconstructs saved trajectories from a finished training run,
segments them by environment switch, and measures how often the agents visit
cells that were barriers in the previous world.
"""
...
def _expand_barrier_mask(mask: np.ndarray, thickness: int) -> np.ndarray:
...
def _derive_barrier_mask(world: np.ndarray, thickness: int) -> np.ndarray:
    """Return a binary mask that highlights barrier cells in ``world``."""
    ...
    # Treat any positive integer tile that is not on the outer boundary as a
    # barrier. Boundary-aligned positives correspond to permanent walls and are
    # therefore excluded. ``np.isclose`` keeps us tolerant of float encodings of
    # the canonical ``1`` barrier label.
    ...
    return _expand_barrier_mask(mask.astype(float), thickness)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyse barrier occupancy")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Include cells within this many Manhattan steps of a barrier (default: 1)",
    )
...
def main() -> None:
    ...
    for seed_key, seed_block in structure.items():
        ...
        seed_summary = {"world_count": 0, "metrics": [], "warnings": []}
        ...
        if worlds is None or getattr(worlds, "size", 0) == 0:
            ...
            seed_summary["warnings"].append("missing_world_data")
            continue
        ...
        if len(flat_worlds) < 2:
            print(f"WARNING: seed {seed_name} has fewer than two worlds; skipping")
            seed_summary["warnings"].append("insufficient_worlds")
            continue
        ...
        if not grouped_paths:
            ...
            seed_summary["warnings"].append("missing_paths")
            continue

        seed_metrics: List[Dict[str, object]] = []
        for world_index in range(1, len(flat_worlds)):
            previous_world = flat_worlds[world_index - 1]
            grid_size = previous_world.shape[0]
            mask = _derive_barrier_mask(previous_world, args.barrier_thickness)
            if mask.shape != previous_world.shape:
                raise ValueError("Barrier mask shape mismatch")
            ...
            for prefix, grouped in grouped_paths.items():
                ...
                barrier_steps = float(np.sum(occupancy * mask))
                occupancy_norm = _normalise(occupancy)
                barrier_fraction = float(np.sum(occupancy_norm * mask))
                ...
                if math.isfinite(barrier_fraction):
                    fraction_accumulator.setdefault(world_index, {}).setdefault(
                        prefix, []
                    ).append(barrier_fraction)

            block = {
                "seed": seed_name,

                "world_index": world_index,
                "previous_world_barrier_cells": mask_sum,
                "episode_window": {
            ...
            aggregate_records.append(
                {
                    ...
                    "agent": agent,
                    "seed_count": int(values.size),
                    "mean_barrier_fraction": mean,
                    "std_barrier_fraction": std,
                }
            )
    ...
    summary = {
        ...
        "aggregate_barrier_fraction": aggregate_fractions,
        ...
        for block in overall_metrics:
            ...
            for agent, stats in block["agents"].items():
                row = [
                    ...
                    f"{stats['barrier_steps']:.6g}",
                    f"{stats['barrier_fraction']:.6g}",
                ]
                handle.write(",".join(row) + "\n")

    aggregate_csv = output_dir / "occupancy_barrier_fraction_summary.csv"
    with aggregate_csv.open("w", encoding="utf-8") as handle:
        header = [
            "world_index",
            "agent",
            "seed_count",
            "mean_barrier_fraction",
            "std_barrier_fraction",
        ]
        handle.write(",".join(header) + "\n")
        for record in sorted(
            ...
            row = [
                ...
                f"{record['mean_barrier_fraction']:.6g}",
                f"{record['std_barrier_fraction']:.6g}",
            ]
            handle.write(",".join(row) + "\n")

    print(f"Saved occupancy metrics to {metrics_json}")
