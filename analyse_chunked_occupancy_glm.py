...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Join per-target chunked occupancy metrics and fit logistic GLMs "
            "to predict agent labels from combined features."
        )
    )
    parser.add_argument(
        "--current-dir",
        ...
        required=True,
        help=(
            "Directory that contains chunked_occupancy_metrics.csv for "
            "current-hole occupancies."
        ),
    )
    parser.add_argument(
        "--previous-dir",
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory that contains chunked_occupancy_metrics.csv for wall occupancies.",
    )
    parser.add_argument(
        "--lag-features-dir",
        type=Path,
        default=None,
        help=(
            "Optional directory used to source lag breakdowns (per-previous-world) for building "
            "flexible features f_holes_world_t_minus_k. If omitted, the script will use --previous-dir. "
            "When a specific lag folder (e.g. t_minus_4_hole_locations) is provided, the script will "
            "auto-detect and use the sibling 'previous_hole_locations' directory if present."
        ),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        required=True,
        help="Directory where the joined dataset and GLM artefacts will be written.",
    )
    parser.add_argument(
        "--positive-label",
        default=_DEFAULT_POSITIVE_LABEL,
        help="Agent label that is treated as the positive class (default: lesionLEC).",
    )
    parser.add_argument(
        "--inverse-regularisation",
        type=float,
        default=1.0,
        help=(
            "Legacy compatibility flag; retained for CLI stability but ignored by the "
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Maximum number of iterations for the statsmodels Logit optimiser (default: 1000).",
    )
    parser.add_argument(
        "--centre-within-seed",
        action="store_true",
        help=(
            "Centre features within each (seed, chunk_index) group before fitting to remove "
            "between-seed offsets."
        ),
    )
    parser.add_argument(
        "--demean-by-world",
        action="store_true",
        help=(
            "Subtract global per-world mean occupancies before fitting so the model focuses on"
            " deviations attributable to agent differences."
        ),
    )
    parser.add_argument(
        "--log-level",
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Number of worker processes to use when fitting chunk models. "
            "Values greater than 1 enable parallel execution (default: 1)."
        ),
    )
