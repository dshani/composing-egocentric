...
#         description=(
#             "Analyse how often agents occupy current-hole, previous-hole, or wall"
#             " regions by aggregating fixed-size episode chunks."
#         )
#     )
#     parser.add_argument(
#         "--job-id",
...
#         default=8,
#         help=(
#             "Maximum worker processes for loading trajectories and per-seed "
#             "chunk aggregation"
#         ),
#     )
#     parser.add_argument(
...
#         action="store_true",
#         help=(
#             "Process trajectory analysis sequentially instead of using worker "
#             "process pools"
#         ),
#         default=True,
#     )
...
#         type=int,
#         default=500,
#         help="Number of episodes aggregated per occupancy chunk (default: 500)",
#     )
#     parser.add_argument(
#         "--occupancy-target",
#         default="current_hole_locations",
#         help=(
#             "World feature to score occupancy against. Use 'all' to generate"
#             " outputs for each available target concurrently."
#         ),
#     )
#     parser.add_argument(
#         "--barrier-thickness",
...
#         default=1,
#         help=(
#             "Include cells within this many Manhattan steps of the hole mask when"
#             " scoring occupancy (default: 1)"
#         ),
#     )
#     parser.add_argument(
#         "--wall-thickness",
...
#         default=1,
#         help=(
#             "Include cells within this many Manhattan steps of the wall mask when"
#             " scoring occupancy (default: 1)"
#         ),
#     )
#     parser.add_argument(
...
#         default=None,
#         help=(
#             "Restrict sample extraction to episodes within the inclusive window."
#             " Provide both the start and end episode indices."
#         ),
#     )
#     parser.add_argument(
#         "--sample-agent-prefix",
#         action="append",
#         dest="sample_agent_prefixes",
#         default=None,
#         help=(
#             "Only collect path-level samples for agents whose prefix matches this"
#             " option. Repeat the flag to include multiple prefixes."
#         ),
#     )
#     parser.add_argument(
#         "--sample-min-start-distance",
#         type=float,
#         default=None,
#         help=(
#             "Discard candidate paths whose starting position lies closer than this"
#             " Manhattan distance to the reward."
#         ),
#     )
#     parser.add_argument(
#         "--sample-occupancy-thresholds",
#         type=float,
#         nargs=2,
#         metavar=("MIN", "MAX"),
#         default=None,
#         help=(
#             "Only keep samples whose occupancy fraction falls within the provided"
#             " inclusive [MIN, MAX] interval. Use NaN-friendly values such as 0 and"
#             " 1 to keep all finite samples."
#         ),
#     )
#     parser.add_argument(
#         "--sample-max-records",
...
#         default=None,
#         help=(
#             "Maximum number of path-level samples to retain per agent prefix."
#             " Omit to keep every qualifying sample."
#         ),
#     )
...
#     available_targets = derive_occupancy_targets(structure)
#     valid_targets = set(available_targets)

#     if args.occupancy_target != "all" and args.occupancy_target not in valid_targets:
#         lag_offset = _lag_offset_for_target(args.occupancy_target)
#         available_display = ", ".join((*available_targets, "all"))
#         if lag_offset is not None and lag_offset >= 1:
#             raise SystemExit(
#                 "Requested lag target exceeds available worlds; "
#                 f"available targets: {available_display}"
#             )
#         raise SystemExit(
#             f"Unknown occupancy target '{args.occupancy_target}'. "
#             f"Available targets: {available_display}"
#         )

#     sample_config: Optional[_SampleConfig] = None
#     if (
#         args.sample_episode_window
#         or args.sample_agent_prefixes
#         or args.sample_min_start_distance is not None
#         or args.sample_occupancy_thresholds
#         or args.sample_max_records is not None
#     ):
#         episode_window: Optional[Tuple[Optional[int], Optional[int]]] = None
#         if args.sample_episode_window:
#             start, end = args.sample_episode_window
#             if end < start:
#                 raise SystemExit("--sample-episode-window expects START <= END")
#             episode_window = (int(start), int(end))

#         agent_prefixes = None
#         if args.sample_agent_prefixes:
#             agent_prefixes = frozenset(str(prefix) for prefix in args.sample_agent_prefixes)

#         occupancy_thresholds: Optional[Tuple[Optional[float], Optional[float]]] = None
#         if args.sample_occupancy_thresholds:
#             lower, upper = args.sample_occupancy_thresholds
#             if (math.isfinite(lower) and math.isfinite(upper)) and upper < lower:
#                 raise SystemExit("--sample-occupancy-thresholds expects MIN <= MAX")
#             occupancy_thresholds = (float(lower), float(upper))

#         max_records = args.sample_max_records
#         if max_records is not None and max_records <= 0:
#             raise SystemExit("--sample-max-records must be positive when provided")

#         sample_config = _SampleConfig(
#             episode_window=episode_window,
#             agent_prefixes=agent_prefixes,
#             min_start_distance=(
#                 float(args.sample_min_start_distance)
#                 if args.sample_min_start_distance is not None
#                 else None
#             ),
...
#     base_output_dir = args.output or job_dir
#     parallel_enabled = not args.disable_parallel

#     if args.occupancy_target == "all":
#         target_sequence = list(available_targets)
#     else:
#         target_sequence = [args.occupancy_target]

#     if args.occupancy_target == "all":
#         target_count = len(target_sequence)
#         if not parallel_enabled:
#             target_pool_workers = 1
#             per_target_workers = args.max_workers
#         elif args.max_workers and args.max_workers > 0:
#             target_pool_workers = max(1, min(target_count, args.max_workers))
#             per_target_workers = max(1, args.max_workers // target_pool_workers)
#         else:
#             target_pool_workers = target_count
#             per_target_workers = None
...
#                     "max_workers": per_target_workers,
#                     "enable_parallel": parallel_enabled,
#                     "sample_config": sample_config,
#                 }
#             )

#         if target_pool_workers == 1:
#             for payload in target_jobs:
#                 _analyse_target(**payload)
#         else:
#             with ProcessPoolExecutor(max_workers=target_pool_workers) as executor:
#                 futures = [
#                     executor.submit(_analyse_target_entrypoint, payload)
#                     for payload in target_jobs
#                 ]
...
#             env_switch_every=args.env_switch_every,
#             chunk_size=args.chunk_size,
#             occupancy_target=target_sequence[0],
#             barrier_thickness=args.barrier_thickness,
#             wall_thickness=args.wall_thickness,
